...
software() {
	...
	# these are needed to run the wifi portal & nvr & mounting ntfs ssd (windows file system)
	apt-get update
	apt-get install -y nodejs network-manager dnsmasq hostapd ffmpeg ntfs-3g fuse
