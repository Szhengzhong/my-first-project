...
PRE_CUSTOMIZE_IMAGE
	...
	display_alert "Calling pre-customization script" "preinst.sh" "info"

	preinst "${SRC}/.." "${SDCARD}" $RELEASE
	PRE_CUSTOMIZE_IMAGE_RC=$?

	if [[ $PRE_CUSTOMIZE_IMAGE_RC != 0 ]]; then
		exit_with_error "preinst.sh exited with error (rc: $PRE_CUSTOMIZE_IMAGE_RC)"
	fi

	cp "${RECIPE}/install.sh" "${SDCARD}/tmp/install.sh"
	...
	chroot "${SDCARD}" /bin/bash -c "/tmp/install.sh $RELEASE $NODE_REPO"
	CUSTOMIZE_IMAGE_RC=$?
	...
	display_alert "Calling post-customization script" "postinst.sh" "info"

	postinst "${SRC}/.." "${SDCARD}" $RELEASE
	POST_CUSTOMIZE_IMAGE_RC=$?

	if [[ $POST_CUSTOMIZE_IMAGE_RC != 0 ]]; then
		exit_with_error "postinst.sh exited with error (rc: $POST_CUSTOMIZE_IMAGE_RC)"
	fi

	rm -f "${SDCARD}/tmp/install.sh"
	rm -f "${SDCARD}"/etc/update-motd.d/*
	rm -f "${SDCARD}"/etc/issue
	rm -f "${SDCARD}"/etc/motd
	rm -f "${SDCARD}"/etc/update-motd.d/10-uname

	mkdir -p "${SDCARD}/etc/update-motd.d"

	install -m 644 "${SRC}/config/motd/issue" "${SDCARD}/etc/issue"
	install -m 644 "${SRC}/config/motd/motd" "${SDCARD}/etc/motd"
	install -m 755 "${SRC}/config/motd/10-uname" "${SDCARD}/etc/update-motd.d/10-uname"
	install -m 755 "${SRC}/config/motd/20-network" "${SDCARD}/etc/update-motd.d/20-network"
