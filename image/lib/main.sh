...
if [[ -z $RECIPE_CODE_NAME ]]; then
	options=()
	recipe_options

	RECIPE_CODE_NAME=$(DIALOGRC="${SRC}/config/dialog.conf" dialog --stdout --keep-tite --title "Choose a build recipe" --backtitle "$BACKTITLE" --menu "Select the desired build recipe" 30 120 $((50 - 8)) "${options[@]}")

	[[ -z $RECIPE_CODE_NAME ]] && exit_with_error "No build recipe selected"

	unset options
fi

RECIPE="${SRC}/../recipes/${RECIPE_CODE_NAME}"

export RECIPE_CODE_NAME
export RECIPE

source "${RECIPE}"/preinst.sh
source "${RECIPE}"/postinst.sh
...
if [[ -z $BOARD ]]; then
	...
	BOARD=$(DIALOGRC="${SRC}/config/dialog.conf" dialog --stdout --keep-tite --title "Choose a board" --backtitle "$BACKTITLE" --scrollbar --colors --menu "Select the target board" 30 120 $((50 - 8)) "${options[@]}")
...
if [[ -z $RELEASE ]]; then
	...
	RELEASE=$(DIALOGRC="${SRC}/config/dialog.conf" dialog --stdout --keep-tite --title "Choose a release package base" --backtitle "$BACKTITLE" --menu "Select the target OS release package base" 30 120 $((50 - 8)) "${options[@]}")
...
if [[ -z $NODE_REPO ]]; then
	...
	NODE_REPO=$(DIALOGRC="${SRC}/config/dialog.conf" dialog --stdout --keep-tite --title "Choose a Node release" --backtitle "$BACKTITLE" --menu "Select the desired Node release branch" 30 120 $((50 - 8)) "${options[@]}")
...
BUILD_PROJECTS=($(cat "${RECIPE}/build"))

for i in "${BUILD_PROJECTS[@]}"
do
	(cd "${SRC}/../${i}" && ./project build)
done
...
export IMG_FILENAME="${RECIPE_CODE_NAME}-v${BUILD_VERSION}-$(echo "${BOARD}" | sed 's/[A-Z]/\L&/g')"
