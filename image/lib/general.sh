...
recipe_menu ()
{
	...
	if [[ -d "${recipe_dir}" ]]; then
		local recipe_codename="$(basename "${recipe_dir}")"
		local recipe_fullname="$(cat "${recipe_dir}/name")"

		options+=("${recipe_codename}" "${recipe_fullname}")
	fi
}

recipe_options ()
{
	for recipe_dir in "${RECIPES_DESC_DIR}/"*; do
		recipe_menu "${recipe_dir}"
	done
}
